...
if __name__ == '__main__':
    ...
    robot_socket.bind((lib.SERVER, lib.ARTEC_PORT))
    # listen for incoming connections
    robot_socket.listen()
    # list of sockets to monitor
    print('Server started, waiting for connections...')
    serverSockets.append(robot_socket)
    
    CLI_socket =  socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    # bind the socket to a specific address and port
    CLI_socket.bind((lib.SERVER, lib.CLI_PORT))
    # listen for incoming connections
    CLI_socket.listen()
    # list of sockets to monitor
    print('Server started, waiting for connections...')
    serverSockets.append(CLI_socket)
    
    CAM_socket =  socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    # bind the socket to a specific address and port
    CAM_socket.bind((lib.SERVER, lib.CAM_PORT))
    # listen for incoming connections
    CAM_socket.listen()
    # list of sockets to monitor
    print('Server started, waiting for connections...')
    serverSockets.append(CAM_socket)
    
    GUI_socket =  socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    # bind the socket to a specific address and port
    GUI_socket.bind((lib.SERVER, lib.GUI_PORT))
    # listen for incoming connections
    GUI_socket.listen()
    # list of sockets to monitor
    print('Server started, waiting for connections...')
    serverSockets.append(GUI_socket)
    
    while True:
        start = time.time()
        read_sockets = []
        read_sockets, write_sockets, error_sockets = select.select(read_sockets.append(serverSockets).append(robotSockets), [], [],0)
        ...
        for sock in read_sockets:
            if sock in serverSockets:
                ...
                if sock == robot_socket:
                    for addr in robots.items():
